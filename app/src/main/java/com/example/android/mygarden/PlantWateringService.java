...
public class PlantWateringService extends IntentService {
    ...
    private void handleActionUpdatePlantWidgets() {
        ...
        int imageId = R.drawable.grass;
        if (cursor != null && cursor.getCount() > 0){
            cursor.moveToFirst();

            int colWateredIndex = cursor.getColumnIndex(
                    PlantContract.PlantEntry.COLUMN_LAST_WATERED_TIME);
            int colCreationIndex = cursor.getColumnIndex(
                    PlantContract.PlantEntry.COLUMN_CREATION_TIME);
            int colPlantTypeIndex = cursor.getColumnIndex(
                    PlantContract.PlantEntry.COLUMN_PLANT_TYPE);

            long timeSinceWatered = System.currentTimeMillis() - cursor.getLong(colWateredIndex);
            long creation = cursor.getLong(colCreationIndex);
            long age = System.currentTimeMillis() - creation;
            int type = cursor.getInt(colPlantTypeIndex);

            imageId = PlantUtils.getPlantImageRes(getApplicationContext(), age, timeSinceWatered,
                    type);
            AppWidgetManager widgetManager = AppWidgetManager.getInstance(this);
            int[] appWidgetIds = widgetManager.getAppWidgetIds(new ComponentName(this,
                    PlantWidgetProvider.class));

            PlantWidgetProvider.updatePlantWidgets(getApplicationContext(), widgetManager,
                    imageId, appWidgetIds);
        }
    }
